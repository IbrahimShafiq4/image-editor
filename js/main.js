...
const loadImage = () => {
    ...
    if (!file) return; // return if user hasn't selected filed
...
rotateOptions.forEach(options => {
    options.addEventListener('click', () => { // adding click event listener to all rotate/flip buttons
        if (options.id === "left") {
            ...
            // if flipVertical is 1, set this value to -1 else set 1
            flipVertical = flipVertical === 1 ? -1 : 1;
        } else {
            // if flipHorizontal is 1, set this value to -1 else set 1
...
const resetFilter = () => {
    // resetting all variable value to it's default value
